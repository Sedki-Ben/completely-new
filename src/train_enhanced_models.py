...
def load_and_preprocess_data(data_dir: str):
    ...
    # Ensure CSI data has the correct shape (samples, time_steps, features)
    if len(csi_features.shape) == 2:
        csi_features = csi_features.reshape(csi_features.shape[0], -1, 1)
    if len(csi_phase.shape) == 2:
        csi_phase = csi_phase.reshape(csi_phase.shape[0], -1, 1)
...
def main():
    ...
    # Set paths
    data_dir = Path('Indoor-Localization-main/data')  # Updated path to match actual location

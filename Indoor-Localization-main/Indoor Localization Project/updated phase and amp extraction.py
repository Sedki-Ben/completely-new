...
def process_csi_file(input_file, output_file=None):
    ...
    if output_file is None:
        ...
        output_file = os.path.join(os.path.dirname(input_file), f"Amp+Phase_{file_name_without_ext}.csv")
    ...
    for idx, row in df.iterrows():
        ...
        for i in range(0, len(csi_data) - 1, 2):
            ...
            # Append to the lists
            amplitudes.append(float(amplitude))  # Convert to regular float
            phases.append(float(phase))         # Convert to regular float
...
if __name__ == "__main__":
    ...
    data_directory = r'C:\MasterArbeit\NewRoom Pipeline\Data\Processed_Data\sub 52 to 72'

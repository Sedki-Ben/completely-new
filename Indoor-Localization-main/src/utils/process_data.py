...
def load_raw_data(data_dir: Path):
    ...
    for csv_file in data_dir.glob('*.csv'):
        try:
            ...
            csi = df.iloc[:, :-1].values  # All columns except last
            rssi = df.iloc[:, -1].values  # Last column
...
def main():
    ...
    processed_data_dir = Path('Indoor-Localization-main/data/processed')
    ...
    np.save(processed_data_dir / 'csi_features.npy', csi_features)
    np.save(processed_data_dir / 'csi_phase.npy', csi_phase)
    np.save(processed_data_dir / 'rssi.npy', rssi_processed)
    np.save(processed_data_dir / 'locations.npy', locations)
